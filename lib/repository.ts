...
export async function listInvestigations():Promise<Case[]>{if(!hasDb())return seedCases;const investigations=await request("investigations?select=*&order=updated_at.desc");if(!investigations.length)return seedCases;return Promise.all(investigations.map(async(i:any)=>{const [nodes,edges,evidence,timeline,questions]=await Promise.all([request(`entities?investigation_id=eq.${encodeURIComponent(i.id)}&select=*`),request(`relationships?investigation_id=eq.${encodeURIComponent(i.id)}&select=*`),request(`evidence?investigation_id=eq.${encodeURIComponent(i.id)}&select=*`),request(`timeline?investigation_id=eq.${encodeURIComponent(i.id)}&select=*&order=id.asc`),request(`open_questions?investigation_id=eq.${encodeURIComponent(i.id)}&resolved=eq.false&select=*`)]);return {id:i.id,name:i.name,domain:i.domain,status:i.status,summary:i.summary,nodes:nodes.map((n:any)=>({id:n.id,label:n.label,type:n.type,subtitle:n.subtitle,x:Number(n.x),y:Number(n.y),confidence:n.confidence,details:n.details||[]})),edges:edges.map((e:any)=>({from:e.from_entity,to:e.to_entity,label:e.label,confidence:e.confidence})),evidence,timeline:timeline.map((t:any)=>({date:t.event_date,title:t.title,body:t.body})),openQuestions:questions.map((q:any)=>q.question)} as Case}))}
